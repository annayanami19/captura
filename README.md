...
- 📸 **Screenshot 4 mode** — langsung dari popup **atau dari jendela screenshot** (tanpa picker) untuk tab aktif: **Penuh** 🖥️, **Full Page** 📄 (seluruh tinggi halaman — pre-scroll otomatis memicu lazy-load, diambil via debugger protocol), **Area** ⬚ (drag kotak di halaman), **Bebas** ✏️ (gambar bentuk bebas); plus layar/jendela via picker.
- 🖌️ **Editor screenshot** — potong area, blur/mosaik ▓, coretan ✏️ multi-warna, **🏷️ watermark** bebas diedit, **zoom 25%–400%** (Ctrl+scroll), **📋 Copy ke clipboard** (ikut semua editan), **🛟 draf otomatis** yang bisa dipulihkan, dan scrollbar halaman otomatis dibuang dari hasil. (Fitur teks 🅣: segera hadir.)
- ⏺ **Kontrol lengkap** — mulai / jeda / lanjut / selesai, timer + estimasi ukuran file, badge REC di toolbar, dan konfirmasi sebelum jendela tertutup saat ada sesi berjalan.
- 🔊 **Audio opsional** — rekam suara microphone, audio sistem/tab, atau keduanya (dicampur otomatis).

- ⚙️ **Halaman Pengaturan 3 kolom** — *Rekam Video* (kiri), *Panduan Cepat* (tengah — bisa di-collapse & posisinya diingat), *Screenshot* (kanan — termasuk opsi "tampilkan penuh layar saat screenshot berhasil"). Semua preferensi tersimpan otomatis di penyimpanan lokal browser dan tersinkron langsung antar jendela.
- 🐞 **Debug Log** — dua tab terpisah (🎥 Captura & 📸 Screenshot) dengan checkbox rekam on/off (default **nonaktif**), maks. 200 entri per fitur, tersimpan lokal di browser.
